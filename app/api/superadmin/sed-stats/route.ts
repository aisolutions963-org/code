...
export const GET = requireRole('superadmin')(async () => {
  const [projects, sedUsers] = await Promise.all([
    ...
    Promise.resolve(
      (db.prepare(`SELECT name FROM users WHERE role = 'sed' AND active = 1 ORDER BY name`).all() as { name: string }[]),
    ),
  ])

  const sedNames = sedUsers.map((u) => u.name)

  // Group counts by SED name
  ...
  for (const p of projects) {
    ...
    if (!map[p.ownerName]) {
      map[p.ownerName] = { preparing: 0, open: 0, closed: 0, notApproved: 0 }
      if (!sedNames.includes(p.ownerName)) sedNames.push(p.ownerName)
    }
    const entry = map[p.ownerName]
