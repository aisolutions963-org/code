...
function DocLinksField({
  ...
  function handleAdd() {
    ...
    setUrlError('')
    onAdd(fieldKey, { url: trimUrl || undefined, label: trimLabel, notes: notesInput.trim() || undefined })
    setUrlInput('')
    setLabelInput('')
    setNotesInput('')
    setExpanded(false)
    ...
    <div className="space-y-2" dir={ar ? 'rtl' : 'ltr'}>
      ...
      {legacyFiles && legacyFiles.length > 0 && (
        <ul className="space-y-1.5">
          {legacyFiles.map((att) => (
            <li key={att.id} className="bg-gray-50 border border-gray-200 rounded-lg px-3 py-2 flex items-center gap-2">
              <svg className="w-3.5 h-3.5 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
                  d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
              </svg>
              <a href={att.url} target="_blank" rel="noopener noreferrer"
                className="text-sm text-brand-600 hover:underline truncate flex-1">{att.filename}</a>
              <span className="text-[10px] text-gray-300 shrink-0">uploaded</span>
            </li>
          ))}
        </ul>
      )}
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  {link.url ? (
                    ...
                    <span className="text-sm text-gray-700 font-medium block">{link.label}</span>
                  )}
                  {link.notes && (
        ...
        return (
          <div key={key} className="space-y-2">
            <DocLinksField
              ...
              ar={ar}
              legacyFiles={oldAttachments.length > 0 ? oldAttachments : undefined}
            />
          </div>
        )
      })}
